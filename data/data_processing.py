...
from inversefed.data.consts import *

#from .loss import Classification, PSNR


def construct_dataloaders(dataset, augmentations=True, data_path='~/data', shuffle=True, normalize=True):
...
def _build_imagenet(data_path, augmentations=True, normalize=True):
    ...
    # Load data
    trainset = torchvision.datasets.ImageFolder("D:\高精尖中心\抗恶意攻击\code\data\mini-imagenet/train", transform=transforms.ToTensor())
    validset = torchvision.datasets.ImageFolder("D:\高精尖中心\抗恶意攻击\code\data\mini-imagenet/val", transform=transforms.ToTensor())
